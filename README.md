...
#### Windows Installer
You can generate the installer with `ninja win-installer` after a successful
build. This assumes a working internet connection and installation of the
optional dependencies.

You can generate the portable zip with `ninja win-portable` after a successful
build.
...
apk add alsa-lib-dev boost-dev ffms2-dev fftw-dev fontconfig-dev gtest-dev \
   hunspell-dev meson uchardet-dev wxwidgets-dev
```

Ubuntu:
```sh
apt-get install intltool libasound2-dev libass-dev libboost-all-dev \
    libffms2-dev libfontconfig1-dev libgtest-dev pkgconf build-essential \
    libfreetype6-dev libwxgtk3.0-gtk3-dev meson
```
